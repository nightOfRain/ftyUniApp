<template>
	<view>
		...
		<view class="flex flex-wrap padding text-xl solid-top text-center bg-white" v-if="mybj.length>0">
			<view class="basis-df padding-bottom-xs">总还款</view>
			<view class="basis-df padding-bottom-xs">总利息</view>
			<view class="basis-df text-black"><text class="text-price">{{amt_total}} 元</text></view>
			<view class="basis-df text-black"><text class="text-price">{{lx_total}} 元</text></view>
		</view>
	...
	export default {
		...
		methods: {
			...
			get_result_bj:function (flag){
				...
				this.amt_total = (bj_total + lx_total).toFixed(2);
			},
			...
			get_result_bx:function (flag){
				...
				yg = (amt*per*this.cifang((1+per), months))/(this.cifang((1+per), months)-1);
		
				for(var i = 0; i < months; i++){
					var lx_tmp = amt*per*(this.cifang((1+per),months) - this.cifang((1+per), i))/(this.cifang((1+per),months) -1.0);
					var bj_tmp = amt*per*(this.cifang((1+per), i))/(this.cifang((1+per),months) -1.0);
		...
		console.log("======mylx:"+JSON.stringify(this.mylx));
